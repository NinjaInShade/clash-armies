...
export const CURRENT_SIEGES: string[] = ['Wall Wrecker', 'Battle Blimp', 'Stone Slammer', 'Siege Barracks', 'Log Launcher', 'Flame Flinger', 'Battle Drill'];
...
export async function insertInitialUnits(db: MySQL) {
	...
	for (const [name, levels] of Object.entries(troopsSeedData)) {
		...
		const unitLevels = Object.entries(levels).map(([level, data]) => {
			...
			if (typeof HousingSpace !== 'number' || typeof TrainingTime !== 'number') {
				throw new Error(
					`Expected number value for static JSON for troop "${name}" (barrackLvl=${BarrackLevel}, labLvl=${LaboratoryLevel}, housingSpace=${HousingSpace}, trainingTime=${TrainingTime})`
				);
			}
	...
	for (const [name, levels] of Object.entries(SpellsSeedData)) {
		...
		const unitLevels = Object.entries(levels).map(([level, data]) => {
			...
			if (typeof HousingSpace !== 'number' || typeof TrainingTime !== 'number') {
				throw new Error(
					`Expected number value for static JSON for spell "${name}" (spellForgeLvl=${SpellForgeLevel}, labLvl=${LaboratoryLevel}, housingSpace=${HousingSpace}, trainingTime=${TrainingTime})`
				);
			}
