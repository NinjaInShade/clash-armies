...
<div class="vote {_class ? _class : ''}">
	...
	<button
		onclick={userVote === -1 ? clearVote : downvote}
		disabled={saving || !allowEdit}
		class="focus-grey"
		class:selected={userVote === -1}
		aria-label="Downvote"
	>
	...
	<button onclick={userVote === 1 ? clearVote : upvote} disabled={saving || !allowEdit} class="focus-grey" class:selected={userVote === 1} aria-label="Upvote">
