...
<div class="vote">
	{#if allowEdit}
		<button onclick={userVote === -1 ? clearVote : downvote} disabled={saving}>
			<svg width="17" height="16" viewBox="0 0 17 16" fill="none" xmlns="http://www.w3.org/2000/svg">
				<path
					...
					fill={userVote === -1 ? 'var(--downvote)' : '#666'}
				/>
			</svg>
		</button>
	{/if}
	<b class={getClass(votes)} class:noEdit={!allowEdit}>{votes}</b>
	{#if allowEdit}
		<button onclick={userVote === 1 ? clearVote : upvote} disabled={saving}>
			<svg width="18" height="16" viewBox="0 0 18 16" fill="none" xmlns="http://www.w3.org/2000/svg">
				<path
					...
					fill={userVote === 1 ? 'var(--upvote)' : '#666'}
				/>
			</svg>
		</button>
	{/if}
