...
<script lang="ts" generics="T">
	type Props = {
		 ...
		 *
		 * If provided, the radio group becomes "controlled" - clicking it calls this, but does not update `value` itself.
		 * The caller has to own this, so beware if using `bind:value` with this.
		 */
		onChange?: (value: T) => Promise<void> | void;
		/** Allows clicking the currently selected option again to clear the selection (value becomes `undefined`) */
		enableDeselection?: boolean;
	};
	let { value = $bindable(), options, disabled, class: _class, style, onChange, enableDeselection = true }: Props = $props();

	// Groups the underlying radio inputs together so native keyboard navigation (arrow keys) works between them
	const groupName = $props.id();

	async function onClick(e: MouseEvent, option: RadioOption<T>) {
		if (disabled) {
			return;
		}
		e.preventDefault();
		if (option.value === value && !enableDeselection) {
			return;
		}
...
<div class="radio-group {_class ?? ''}" {style}>
	{#each options as option, i (i)}
		{@const checked = option.value === value}
		<label class="radio-option" class:disabled>
			<input type="radio" class="radio-input" name={groupName} {checked} {disabled} onclick={(e) => onClick(e, option)} />
			<span class="inner-container" class:checked>
				{#if checked}
...
<style>
	...
	*/
	...
	.radio-input:active + .inner-container,
	.radio-input:focus-visible + .inner-container,
	.radio-option:not(:has(.inner-container.checked)):not(:has(.radio-input:disabled)):hover .inner-container {
		outline: none;
