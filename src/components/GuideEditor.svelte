<script lang="ts">
	...
	let imgInput = $state<string | null>(null);
	let imgMenuOpen = $state(false);
	let imgRef = $state<HTMLButtonElement | undefined>();

	let vidInput = $state<string | null>(null);
	let vidMenuOpen = $state(false);
	let vidRef = $state<HTMLButtonElement | undefined>();

	let linkInput = $state<string | null>(null);
	let linkMenuOpen = $state(false);
	let linkRef = $state<HTMLButtonElement | undefined>();

	const editorExtensions = getGuideEditorExtensions({
		addImgCb: () => {
			toggleImgMenu();
			return true;
		},
	...
	function toggleVidMenu() {
		vidMenuOpen = !vidMenuOpen;
	}
	...
	async function setImg() {
		if (imgInput && imgInput.length) {
			requireEditor().chain().focus().setImage({ src: imgInput }).run();
		}
		imgMenuOpen = false;
		imgInput = null;
	}
	async function setVid() {
		if (vidInput && vidInput.length) {
			requireEditor().commands.setYoutubeVideo({ src: vidInput });
		}
		vidMenuOpen = false;
		vidInput = null;
	}
...
{#if editor}
	<div class="controls">
		...
		<div class="separator"></div>
		<div class="controls-group">
			<button onclick={toggleImgMenu} class="focus-grey" title="Alt+I" bind:this={imgRef}>
				<svg width="13" height="13" viewBox="0 0 13 13" fill="none" xmlns="http://www.w3.org/2000/svg">
					<path d="M2.16667 10.1111H10.8333L8.125 6.5L5.95833 9.38889L4.33333 7.22222L2.16667 10.1111ZM0 13V0H13V13H0Z" fill="#E1E1E1" />
				</svg>
			</button>
			<button onclick={toggleVidMenu} class="focus-grey" title="Alt+V" bind:this={vidRef}>
				<svg width="15" height="13" viewBox="0 0 15 13" fill="none" xmlns="http://www.w3.org/2000/svg">
					<path
						fill-rule="evenodd"
						clip-rule="evenodd"
						d="M7.5 0.5C8.14125 0.5 8.799 0.5165 9.4365 0.5435L10.1895 0.5795L10.9103 0.62225L11.5853 0.668L12.2017 0.716C12.8709 0.766918 13.5006 1.05233 13.98 1.52195C14.4593 1.99158 14.7576 2.6153 14.8223 3.28325L14.8522 3.602L14.9085 4.2845C14.961 4.99175 15 5.76275 15 6.5C15 7.23725 14.961 8.00825 14.9085 8.7155L14.8522 9.398L14.8223 9.71675C14.7576 10.3848 14.4592 11.0086 13.9797 11.4783C13.5002 11.9479 12.8703 12.2333 12.201 12.284L11.586 12.3312L10.911 12.3777L10.1895 12.4205L9.4365 12.4565C8.79137 12.4844 8.14573 12.4989 7.5 12.5C6.85427 12.4989 6.20863 12.4844 5.5635 12.4565L4.8105 12.4205L4.08975 12.3777L3.41475 12.3312L2.79825 12.284C2.12911 12.2331 1.4994 11.9477 1.02003 11.478C0.540666 11.0084 0.242393 10.3847 0.17775 9.71675L0.14775 9.398L0.0915 8.7155C0.0343746 7.97834 0.00385473 7.23936 0 6.5C0 5.76275 0.039 4.99175 0.0915 4.2845L0.14775 3.602L0.17775 3.28325C0.242368 2.61541 0.540534 1.99179 1.01975 1.52218C1.49896 1.05257 2.12849 0.767087 2.7975 0.716L3.41325 0.668L4.08825 0.62225L4.80975 0.5795L5.56275 0.5435C6.20813 0.515545 6.85402 0.501042 7.5 0.5ZM6 4.68125V8.31875C6 8.66525 6.375 8.88125 6.675 8.70875L9.825 6.89C9.89354 6.85054 9.95048 6.79372 9.99007 6.72525C10.0297 6.65678 10.0505 6.57909 10.0505 6.5C10.0505 6.42091 10.0297 6.34322 9.99007 6.27475C9.95048 6.20628 9.89354 6.14946 9.825 6.11L6.675 4.292C6.60657 4.25249 6.52894 4.2317 6.44993 4.23171C6.37091 4.23172 6.29329 4.25254 6.22487 4.29207C6.15646 4.3316 6.09966 4.38845 6.06018 4.4569C6.02071 4.52535 5.99995 4.60298 6 4.682V4.68125Z"
						fill="#E1E1E1"
					/>
				</svg>
			</button>
			<C.Menu bind:open={imgMenuOpen} elRef={imgRef} onClose={setImg}>
				<C.FocusTrap>
					<div class="menu">
						<C.Input bind:value={imgInput} onkeydown={(ev) => ev.key === 'Enter' && setImg()} placeholder="https://imgur.com/..." />
						<C.ActionButton onclick={setImg} theme="grey">Set</C.ActionButton>
					</div>
				</C.FocusTrap>
			</C.Menu>
			<C.Menu bind:open={vidMenuOpen} elRef={vidRef} onClose={setVid}>
				<C.FocusTrap>
					<div class="menu">
						<C.Input bind:value={vidInput} onkeydown={(ev) => ev.key === 'Enter' && setVid()} placeholder="https://youtube.com/..." />
						<C.ActionButton onclick={setVid} theme="grey">Set</C.ActionButton>
					</div>
				</C.FocusTrap>
			</C.Menu>
		</div>
