<script lang="ts">
	...
	const pageMeta = $derived(ARMY_PAGES.hero(data.name));
</script>

<svelte:head>
	<title>ClashArmies • Best {data.name} armies - Clash of Clans</title>
	<meta name="description" content="Browse top performing {data.name} armies. Find the best Clash of Clans strategies featuring the {data.name} hero." />
	<link rel="canonical" href="https://clasharmies.com/armies/heroes/{slug}" />
</svelte:head>
